...
class SMS {
  constructor(data = {}) {
    ...
    this.date_sent = data.date_sent || data.dateSent || '';
    this.protocol = data.protocol || 0;
    this.read = data.read || 0;
    this.status = data.status || 0;
    this.type = data.type || 0;
    this.reply_path_present = data.reply_path_present || data.replyPathPresent || 0;
    this.body = data.body || '';
    this.service_center = data.service_center || data.serviceCenter || '';
    this.locked = data.locked || 0;
    this.sub_id = data.sub_id || data.subId || 0;
    this.error_code = data.error_code || data.errorCode || 0;
    this.creator = data.creator || '';
    this.seen = data.seen || 0;
    this.ipmsg_id = data.ipmsg_id || data.ipmsgId || '';
    this.omeId = data.omeId || '';
    this.raw_data = data.raw_data || data.rawData || '';
  ...
  static create(data) {
    ...
    const model = new SMS(data);
    return new Promise((resolve, reject) => {
      db.transaction(tx => {
        tx.executeSql(
          `INSERT INTO sms
            (id, thread_id, address, date, date_sent, protocol, read, status, type,
             reply_path_present, body, service_center, locked, sub_id, error_code, creator,
             seen, ipmsg_id, omeId, raw_data, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            model.id,
            model.thread_id,
    ...
    return new Promise((resolve, reject) => {
      db.transaction(tx => {
        tx.executeSql(
          ...
          [id],
          (_, { rows }) => {
            if (rows.length > 0) resolve(new SMS(rows.item(0)));
            else resolve(null);
          },
          (_, error) => reject(error)
        );
  ...
  static findAll() {
    ...
    return new Promise((resolve, reject) => {
      db.transaction(tx => {
        tx.executeSql(
          'SELECT * FROM sms ORDER BY created_at DESC',
          [],
          (_, { rows }) => {
            const list = [];
            for (let i = 0; i < rows.length; i++) list.push(new SMS(rows.item(i)));
            resolve(list);
          },
          (_, error) => reject(error)
        );
