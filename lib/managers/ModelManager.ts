...
export class ModelManager {
  ...
  private constructor() {
    ...
    this.status = {
      ...
      loadProgress: 0,
      error: null,
      modelPath: "/storage/emulated/0/Documents/gemma.task",
      backend: null,
      loadTime: null,
    };
    this.config = {
  ...
  async unloadModel(): Promise<boolean> {
    try {
      ...
      const result = await GemmaBridgeModule.unloadModel();
  ...
  async checkModelStatus(): Promise<boolean> {
    try {
      const isLoaded = await GemmaBridgeModule.isModelLoaded();
  ...
  async getDeviceCapabilities(): Promise<DeviceCapabilities> {
    try {
      return await GemmaBridgeModule.getDeviceInfo();
    } catch (error) {
